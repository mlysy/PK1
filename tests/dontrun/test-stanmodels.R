# make sure stan posteriors are correct
...
pk1.init.ls <- list(pk1.init(effect = "Mixed", DE = "SDE",
                             ...
                             sdCl = sdCl, sdKa = sdKa, sdKe = sdKe,
                             Xt = Xt))
PK1.Full.Fit <- sampling(object = pk1.mixed.sde.noise,
                    data = pk1.data, chains = 1,
                    init = pk1.init.ls, iter = 100)

#--- compare stan and R likelihoods ---------------------------------------------

nsamples <- nrow(as.data.frame(PK1.Full.Fit)) # number of posterior samples
